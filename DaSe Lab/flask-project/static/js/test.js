...
{
    console.log("gettable call ");
    hNodes=[];
    // clickedValue=document.getElementById("subj").innerHTML;
	     // getData();
	     let table = document.getElementById('st');
        ...
        if(inputslen > 0)
        {
        	let inputnode = trimOut(inputs[0].value);
            hNodes.push(inputnode);
        }
 ...
 else
 {
    ...
    try
    {
       ...
       for (var i = 0; i < rows.length; i++)
       {
        ...
        console.log("h nodes ",hNodes.length);
        for (let i = 0; i < hNodes.length; i++) {

            temp=this["v"+hNodes[i]];
